...
export function MyComp() {
    ...
    const [first, second] = colors; //배열의 구조분할당 [] 대괄호사용
    const user = {
        name: "Steve",
        age: 25,
    };
    ...
    // 배열의 새로운 데이터를 추가하는데 항상 제일 앞에 추가
    const array4 = [10, ...array3];
    console.log(array4);

    // 배열의 새로운 데이터를 추가하는데 항상 제일 뒤에 추가
    ...
    const itemList = [
        { id: 1, name: "사과", description: "맛있는 사과" },
    ...
    return (
        <>
            ...
            {isLoggendIn ? <p>당신은 로그인 상태 입니다.</p> : <p>로그인 해주세요.</p>}
            {isLoggendIn && <p>true면 보여주세요.</p>}
            <h2>과일목록</h2>
            {itemList.map((item) => (
                <p key={item.id}>
